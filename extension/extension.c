...
bool ZEROS(Program* p, Data* d);
bool ONES(Program* p, Data* d);
bool RAND(Program* p, Data* d);
...
bool SUB(Data* d, int i,int j, int num, int p_id, int c_row, int c_col);
...
void Instrc(Program* p, Data* d)
{
    ...
    else if(strsame(s, "ONES") || strsame(s, "READ") || strsame(s, "ZEROS") || strsame(s,"RAND")){
        Create(p,d);
        return;
    }
...
void Binaryop(Program* p, Data* d)
{
    ...
    else if (strsame(s, "B-ADD") || strsame(s, "B-TIMES") || strsame(s, "B-EQUALS") || strsame(s,"B-SUB"))
    {
        if (has_2operands(p))
        {
            assert(BOP(p,d));
...
void Create(Program* p, Data* d)
{
    ...
    else if (strsame(s, "ZEROS"))
    {
        assert(ZEROS(p,d));
        return;
    }
    else if (strsame(s,"RAND"))
    {
        assert(RAND(p,d));
        return;
    }
...
void SIZE(Program* p,Data* d)
{
    if (is_var(p)){
        int index = Varname(p,d);
        int total = (d+index)->row * (d+index)->col;
        printf("ROW: %d  Col: %d  TOTAL CELLS: %d\n", (d+index)->row,(d+index)->col, total);
        return;
    }
    else{
        fprintf(stderr, "%s is not a varname. A valid varname is required for SIZE.\n",p->wds[p->cw]);
        exit(EXIT_FAILURE);
    }
}
    ...
    if (has_validop(p,d)){
        for (int i=1; i<=p_row; i++){
            for (int j=1; j<=p_col; j++)
            {
                ...
                if (strsame(s,"B-SUB")){
                    assert(SUB(d,i,j,num,p_id,c_row,c_col));
                }
            }
        }
    }
...
bool is_set(Program* p,Data* d, int index) //check if a varname has been set or not
{
    if (p==NULL || d==NULL)
    {
        return false;
    ...
    if ((!strsame(s0,"SET")) && (!strsame(s0,"LOOP")) && (!strsame(s1,"READ")) && (!strsame(s2,"ONES")) && (!strsame(s2,"ZEROS"))&&(!strsame(s2,"RAND")))
    {
...
void test()
{
    ...
    assert(!ZEROS(NULL, NULL));
    assert(!ONES(NULL,NULL));
    assert(!RAND(NULL,NULL));
}
